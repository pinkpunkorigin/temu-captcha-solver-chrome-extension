...
(function() {
	...
	let semShapesUrl = "https://www.sadcaptcha.com/api/v1/semantic-shapes?licenseKey="
	...
	const SEMANTIC_SHAPES_UNIQUE_IDENTIFIERS = [SEMANTIC_SHAPES_IFRAME]
	...
	*/
	...
	async function solveArcedSlide(): Promise<void> {
		for (let i = 0; i < 3; i++) {
			...
			const endX = getElementCenter(slideButtonEle).x
			let solutionDistanceBackwards = endX - startX - solution
			for (
					let i = 0;
	...
	async function getSlidePieceTrajectory(slideButton: Element, puzzle: Element): Promise<Array<TrajectoryElement>> {
		...
		for (let pixel = 0; pixel < slideBarWidth; pixel++) {
			await new Promise(r => setTimeout(r, 10));
	...
	async function solveSemanticShapes(): Promise<void> {
		for (let i = 0; i < 3; i++) {
			...
			let ele = await waitForElement(SEMANTIC_SHAPES_IMAGE, SEMANTIC_SHAPES_IFRAME)
	...
	async function solveCaptchaLoop() {
		const _: Element = await waitForAnyElementInList(CAPTCHA_PRESENCE_INDICATORS)
